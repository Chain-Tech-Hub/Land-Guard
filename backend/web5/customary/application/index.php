...
switch ($method) {
    case 'GET':
        // Retrieve all lands with owner id of 1 and land status of 2
        $owner_id =1;
        $land_status = 2;
        $type = "Customary";
       
            $stmt = $pdo->prepare('SELECT * FROM land WHERE type = :type and land_status=:land_status');
            $stmt->execute(['type'=>$type, 'land_status'=> $land_status]);
